...
namespace DrawingTest
{
    public partial class Form1 : Form
    {
        ...
        Graphics g;
        Bitmap saveImg;
        Pen pen = new Pen(Color.FromArgb(settings.penColor), settings.penSize);
        bool doDraw = false;
        ...
        /// <summary>
        /// Allows reading of Version 1 DRAW files
        /// </summary>
        /// <param name="br">BinaryReader to allow continual reading of the currently opened file</param>
        private void openVer1(BinaryReader br)
        {
            if (br.ReadByte() != 8) // read the entry length
            {
                MessageBox.Show("Error: Invalid entry length", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            short numEntries = br.ReadInt16(); // self explanatory
            pen.Color = Color.FromArgb(br.ReadInt32());
            ...
            isDrawing = true;
            drawVer1(false); // Don't wait
            isDrawing = false;
        }

        /// <summary>
        /// Allows drawing of Version 1 DRAW files
        /// </summary>
        /// <param name="doWait">True to wait between points, false otherwise</param>
        private void drawVer1(bool doWait)
        {
            ...
            for (int i = 0; i < path.Count; i++)
            {
                ...
                {
                    int index = (i == 0) ? i : i - 1;
                    lastPoint = path[index].coor;
                    thisPoint = path[i].coor;
                    pen.Width = path[i].penSize;
                    listBox1.SelectedIndex = i;

                    g.DrawLine(pen, lastPoint, thisPoint);
                    if (doWait)
                        System.Threading.Thread.Sleep(path[i].wait);
        ...
        private void changePenColorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!isDrawing)
            {
                colorDialog1.Color = Color.FromArgb(settings.penColor);
                if (colorDialog1.ShowDialog() == DialogResult.OK)
                {
                    settings.penColor = colorDialog1.Color.ToArgb();
                    settings.Save();
                    pen.Color = Color.FromArgb(settings.penColor);
                }
            }
        }
        #endregion
        ...
        private void Form1_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left && doDraw == false && !isDrawing)
            {
                if (openedLegacy)
                {
                    MessageBox.Show("Sorry, legacy drawings don't support adding new points in this version, they are opened as read-only.");
                    return;
                }
                short waitTime = 0;
                if (mouseUp)
